...
2. I made a big table of ECP5 IO banks, pin/ball names, schematic signal names,
   ...
   I wrote another script, [triple-check-lpf.py](tripple-check-lpf.py) to
   compare my pin configurations generated by `gen_lpf.py` with the pin configs
   in `verilog/orangecrab_r0.2.1.pcf` from the orangecrab-examples repo. The
   TL;DR version is, after fixing one discrepancy, the configs are close enough
...
8. Writing lpf and Verilog files to put a pullup on `IO_SDA`...
   ...
   Still feeling paranoid about overlooking something, I wrote another script,
   [triple-check-lpf.py](tripple-check-lpf.py) to compare my pin configurations
   generated by `gen_lpf.py` with the pin configs in
   `verilog/orangecrab_r0.2.1.pcf` from the orangecrab-examples repo.

   My triple-check script found one discrepancy for `V17` (`FPGA_RESET` output)
   where the ECP5's `PROGRAMN` reset input is wired to an output pin. I had
   initially filtered out everything that matched `FPGA_RESET`, but now I've
   fixed that so the `V17` pin makes it into the lpf file.

   The full comparison report from my script is in
   [triple-check-report.txt](triple-check-report.txt). Most of the pins agree
   on `IO_TYPE` values. The ones that differ don't matter because they're still
   the same voltage (1.35V), so there should be no trouble with frying the DRAM
   chip.
